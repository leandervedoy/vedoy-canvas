...
import { ArrowRight, BookOpen, Brush, ChevronDown, ChevronRight, Circle, Clipboard, Diamond, Download, Eraser, ExternalLink, FileDown, FilePlus2, FileText, FileType2, FileUp, FolderPlus, Frame, Hand, HelpCircle, Image as ImageIcon, LibraryBig, List, ListOrdered, LogIn, LogOut, Menu, Minus, Moon, MousePointer2, NotebookTabs, Pencil, Presentation, Redo2, Save, Square, Star, StickyNote, Sun, Table2, Trash2, Type, Undo2 } from 'lucide-react'
...
export default function Page() {
  ...
  const bookTextInsert = (html: string) => {
    const selection = window.getSelection()
    if (!selection?.rangeCount) return
    document.execCommand('insertHTML', false, html)
    document.querySelector('[contenteditable="true"]')?.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'insertText' }))
  }
  ...
  const setCanvasColor = (kind: 'stroke' | 'background', color: string) => {
    const api = editorApi.current
    if (!api) return
    api.updateScene({ appState: { [kind === 'stroke' ? 'currentItemStrokeColor' : 'currentItemBackgroundColor']: color } } as never)
  }
  ...
  return <main className="relative min-h-screen overflow-hidden bg-background text-foreground"><style jsx global>{`.excalidraw .App-menu_top__left, .excalidraw .App-menu_top__right, .excalidraw .App-toolbar-container { display: none !important; }`}</style>
    ...
    <header className="absolute inset-x-4 top-4 z-10 flex items-center gap-1.5 rounded-xl border border-border/70 bg-card/95 px-2.5 py-2.5 shadow-md backdrop-blur-md sm:inset-x-6 sm:gap-2 sm:px-3">
      ...
      {viewMode === 'book' && <div className="hidden min-w-0 items-center gap-1 overflow-x-auto md:flex" aria-label="Tekstverktøy"><select defaultValue="Arial" onMouseDown={(event) => event.preventDefault()} onChange={(event) => bookTextCommand('fontName', event.target.value)} className="h-8 max-w-24 rounded-md border border-border bg-transparent px-1 text-[11px]" aria-label="Velg font"><option>Arial</option><option>Georgia</option><option>Verdana</option><option>Courier New</option></select><select defaultValue="3" onMouseDown={(event) => event.preventDefault()} onChange={(event) => bookTextCommand('fontSize', event.target.value)} className="h-8 w-16 rounded-md border border-border bg-transparent px-1 text-[11px]" aria-label="Velg skriftstørrelse"><option value="1">10 px</option><option value="2">12 px</option><option value="3">14 px</option><option value="4">16 px</option><option value="5">20 px</option><option value="6">28 px</option><option value="7">36 px</option></select>{[['bold', 'B'], ['italic', 'I'], ['underline', 'U'], ['strikeThrough', 'S']].map(([command, label]) => <button key={command} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextCommand(command)} className="flex size-8 items-center justify-center rounded-md text-xs font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={command}>{label}</button>)}{['h1', 'h2', 'h3'].map((heading) => <button key={heading} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextCommand('formatBlock', heading)} className="rounded-md px-2 py-1.5 text-[11px] text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={`Overskrift ${heading.slice(1)}`}>{heading.toUpperCase()}</button>)}<button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextCommand('insertUnorderedList')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Punktliste"><List className="size-4" /></button><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextCommand('insertOrderedList')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Nummerliste"><ListOrdered className="size-4" /></button><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextInsert('<mark>Nøkkelord</mark>')} className="rounded-md px-2 py-1.5 text-[11px] font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Nøkkelord">KEY</button><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => bookTextInsert('<table><tbody><tr><th>Kolonne 1</th><th>Kolonne 2</th></tr><tr><td>Verdi</td><td>Verdi</td></tr></tbody></table><p><br></p>')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Tabell"><Table2 className="size-4" /></button></div>}
      ...
      {viewMode === 'canvas' && <><label className="hidden size-8 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-muted sm:flex" title="Strekfarge"><span className="sr-only">Velg strekfarge</span><input type="color" defaultValue="#1f2937" onChange={(event) => setCanvasColor('stroke', event.target.value)} className="size-5 cursor-pointer rounded border-0 bg-transparent p-0" aria-label="Velg strekfarge" /></label><label className="hidden size-8 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-muted sm:flex" title="Bakgrunnsfarge"><span className="sr-only">Velg bakgrunnsfarge</span><input type="color" defaultValue="#ffffff" onChange={(event) => setCanvasColor('background', event.target.value)} className="size-5 cursor-pointer rounded border-0 bg-transparent p-0" aria-label="Velg bakgrunnsfarge" /></label><button type="button" onClick={() => chooseTool('image')} className="hidden size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted sm:flex" aria-label="Legg til bilde" title="Legg til bilde"><ImageIcon className="size-4" /></button><button type="button" onClick={() => chooseTool('embeddable')} className="hidden size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted sm:flex" aria-label="Legg til HTML-innhold" title="Legg til HTML-innhold"><ExternalLink className="size-4" /></button></>}
    ...
    {authReady && !user && <div className="pointer-events-none absolute inset-x-0 bottom-6 z-10 mx-auto w-fit rounded-full border border-border/70 bg-card/95 px-4 py-2 text-xs text-muted-foreground shadow-md">Lagret lokalt · logg inn for skybackup</div>}
